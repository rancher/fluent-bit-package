...
all:
	...
	$(SED) -i 's/docker pull fluent\/fluent-bit:x86_64-.*/docker pull fluent\/fluent-bit:x86_64-$(FULL_VERSION)"/g' $(MAKEPATH)/.drone.yml
	$(SED) -i 's/docker pull fluent\/fluent-bit:arm64v8-.*/docker pull fluent\/fluent-bit:arm64v8-$(FULL_VERSION)"/g' $(MAKEPATH)/.drone.yml
	$(SED) -i 's/docker pull fluent\/fluent-bit:arm32v7-.*/docker pull fluent\/fluent-bit:arm32v7-$(FULL_VERSION)"/g' $(MAKEPATH)/.drone.yml
